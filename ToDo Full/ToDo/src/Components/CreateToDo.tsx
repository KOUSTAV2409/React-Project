...
const CreateToDo = () => {
  ...
  const handleDelete = (index: number) => { // index should be a number type
    const updatedTasks = addButton.filter((_, i) => i !== index);
    setAddButton(updatedTasks);
  };

  const handleKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => { // event should be keyboardEvent
    if (e.key === "Enter") {
      handleClick()
    }
  }
